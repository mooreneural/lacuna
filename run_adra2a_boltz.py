"""ADRA2A (P08913) Boltz-2 ensemble screen -- 30 diffusion samples."""
import sys, time
...
pdb_path = Path(r"C:\Users\clayt\Documents\GitHub\lacuna\benchmarks\pdb_cache\ADRA2A_AF.pdb")
out_dir  = Path(r"C:\Users\clayt\Documents\GitHub\lacuna\adra2a_boltz_lacuna")
...
print(f"ADRA2A (P08913): {n_res} residues, {n_atoms} atoms")
...
if coord_sets_cache.exists():
    ...
    boltz_time = 0
    print(f"Loaded {len(coord_sets)} conformers from cache")
else:
...
print("\nKnown ADRA2A pharmacological sites (Ballesteros-Weinstein):")
known_sites = {
    'orthosteric_catecholamine': {113, 200, 203, 204, 207, 310, 311},
    'na_allosteric':             {79, 80, 81, 82},
    'intracellular_Gi':          {130, 131, 132, 133, 134, 215, 295, 300},
    'ecl2_vestibule':            {163, 164, 165, 166, 167, 168, 169, 170},
}
for site_name, ref_res in known_sites.items():
    ...
    print(f"  {status}  {site_name:28s}  overlap={best_overlap:.0%}  rank={best_rank}  persist={best_persist:.0%}")
